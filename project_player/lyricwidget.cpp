...
bool LyricWidget::process(QString filePath)
{
    ...
    bool isOk = lyricFile.open(QFile::ReadOnly);
    if(!isOk)
    {
        //先清空歌词
        lines.clear();
    ...
    if (pos == -1) {
        ...
        do {
            ...
            if (pos == -1) {
                ...
                foreach (const int& time, timeLabels)
                {
                   // qDebug() <<text.toUtf8().data();
                    lines.push_back(LyricLine(time, text.toUtf8()));
                }
            ...
            if (!text.isEmpty()) {
                ...
                {
                   // qDebug() <<text.toUtf8().data();
                    lines.push_back(LyricLine(time, text.trimmed().toUtf8()));
                }
...
void LyricWidget::show(qint64 position)
{
    ...
    if(index==-1){
        ui->label_3i->setText("");
        ui->label_2i->setText(u8"当前歌曲无歌词");
        ui->label_1i->setText("");
        ui->label_i->setText("");
        ui->label_i1->setText("");
        ui->label_i2->setText("");
